...
- (void)setDoneAssets:(NSMutableArray *)doneAssets{
    _doneAssets = doneAssets;//[NSMutableArray arrayWithArray:doneAssets];
...
- (void)refreshAsset{
    ...
    self.makeView.text = [NSString stringWithFormat:@"%ld",(unsigned long)self.doneAssets.count];
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.photos.count) {
        ...
        MLSelectPhotoAssets *photo = self.deletePhotos[indexPath.item]; //[self.dataSource photoBrowser:self photoAtIndex:indexPath.item];
...
- (void)setPageLabelPage:(NSInteger)page{
    self.title = [NSString stringWithFormat:@"%ld / %ld",page + 1, self.deletePhotos.count];
}
